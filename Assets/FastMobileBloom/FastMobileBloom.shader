...
{
	fixed2 uvOffset = fixed2(0, 0);
	fixed colR;
	fixed colG;
	fixed colB;
	float scanLinePos = 1 - fmod(_Time[0], 0.2) / 0.2;

#if UNITY_UV_STARTS_AT_TOP
	uvOffset.x += 0.01 * float(i.uv2.y - 0.1 < scanLinePos && i.uv2.y + 0.1 > scanLinePos) * sin((i.uv.y + _Time[0]) * 1000);
	colR = tex2D(_MainTex, i.uv2 + uvOffset).x;
	colR += tex2D(_BloomTex, i.uv + uvOffset).x * _BloomIntensity;

	uvOffset.y = 0.01;
	colG = tex2D(_MainTex, i.uv2 + uvOffset).y;
	colG += tex2D(_BloomTex, i.uv + uvOffset).y * _BloomIntensity;

	uvOffset.y = 0.02;
	colB = tex2D(_MainTex, i.uv2 + uvOffset).z;
	colB += tex2D(_BloomTex, i.uv + uvOffset).z * _BloomIntensity;

	return fixed4(colR, colG, colB, tex2D(_BloomTex, i.uv).w + tex2D(_MainTex, i.uv).w);
#else
	uvOffset.x = 0.01 * float(i.uv.y - 0.1 < scanLinePos && i.uv.y + 0.1 > scanLinePos) * sin((i.uv.y + _Time[0]) * 1000);
	colR = tex2D(_MainTex,  i.uv + uvOffset).x;
	colR += tex2D(_BloomTex, i.uv + uvOffset).x * _BloomIntensity;

	uvOffset.y = 0.01;
	colG = tex2D(_MainTex, i.uv + uvOffset).y;
	colG += tex2D(_BloomTex, i.uv + uvOffset).y * _BloomIntensity;

	uvOffset.y = 0.02;
	colB = tex2D(_MainTex, i.uv + uvOffset).z;
	colB += tex2D(_BloomTex, i.uv + uvOffset).z * _BloomIntensity;
#endif
	return fixed4(colR, colG, colB, tex2D(_BloomTex, i.uv).w + tex2D(_MainTex, i.uv).w);
}
